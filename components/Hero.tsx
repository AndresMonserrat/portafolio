"use client";

import { MapPin } from "lucide-react";
import { motion } from "motion/react";
import { profile } from "@/lib/data";
import { fadeUp, fadeUpSm, stagger, typewriter, typewriterContainer } from "@/lib/animations";

export default function Hero() {
  return (
    <header id="inicio" className="scroll-mt-24">
      <motion.div
        className="flex flex-col items-start"
        variants={stagger}
        initial="hidden"
        whileInView="show"
        viewport={{ once: true, amount: 0.2 }}
      >
        <motion.span
          variants={fadeUpSm}
          className="inline-flex items-center gap-2 rounded-full border border-panel-border bg-panel px-3 py-1"
        >
          <span className="h-1.5 w-1.5 rounded-full bg-accent" aria-hidden="true" />
          <motion.span
            className="font-mono text-xs text-foreground"
            variants={typewriterContainer}
            initial="hidden"
            whileInView="show"
            viewport={{ once: true }}
          >
            {profile.available.split("").map((char, i) => (
              <motion.span key={i} variants={typewriter}>
                {char}
              </motion.span>
            ))}
          </motion.span>
        </motion.span>
                
        <motion.h1
          variants={fadeUp}
          className="mt-6 font-display text-5xl font-bold tracking-tight text-foreground lg:text-7xl"
        >

          {profile.name.split("").map((char, i) => (
              <motion.span key={i} variants={typewriter}>
                {char}
              </motion.span>
            ))}
        </motion.h1>

        <motion.p variants={fadeUp} className="mt-3 font-mono text-sm text-muted">
          {profile.metadata} · {profile.university} ·{" "}
          <span className="text-accent">{profile.semester}</span>
        </motion.p>

        <motion.blockquote
          variants={fadeUp}
          className="mt-10 max-w-xl border-l-2 border-accent pl-5 text-lg leading-relaxed text-foreground"
        >
          {profile.quote}
        </motion.blockquote>

        <motion.p
          variants={fadeUp}
          className="mt-6 inline-flex items-center gap-2 font-mono text-xs text-muted"
        >
          <MapPin className="h-3.5 w-3.5" aria-hidden="true" />
          {profile.location}
        </motion.p>
      </motion.div>
    </header>
  );
}